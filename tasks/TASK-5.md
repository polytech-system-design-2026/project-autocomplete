...
6. **Рендер конфига при старте.** Alertmanager не подставляет переменные окружения в свой конфиг, поэтому перед его стартом одноразовый контейнер подставляет значения из `.env` в шаблон и кладёт готовый файл в общий volume:
...
8. **Проверьте руками:**
   ...
   Через минуту с небольшим (`for: 1m` у `AppDown` плюс `group_wait`) бот пришлёт сообщение. Сделайте скриншот. Затем `docker compose start app` — придёт сообщение о разрешении алерта.
...
PR `develop → main`, reviewer `vladefr97`, скриншот сообщения бота — в описании PR. Сообщение в чат: «project-autocomplete, ник, этап 5, PR готов». Подробно — в `Договоренности.md` курса.
...
- **Alertmanager не стартует с «missing chat_id»** — в `.env` `TELEGRAM_CHAT_ID` пустой или 0.

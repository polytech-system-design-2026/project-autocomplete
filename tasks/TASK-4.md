...
Сделать сервис наблюдаемым: по `request_id` найти в логах любой запрос, по метрикам увидеть RPS, задержки и ошибки на дашборде, получить алерт, когда сервис лёг или начал сыпать ошибками.
...
2. **`X-Request-ID` и JSON-логи.** Middleware FastAPI:
   ...
   Бизнес-события — например, «загружен словарь» — тоже пишутся в лог с `request_id`. Удобно хранить текущий `request_id` в `contextvars.ContextVar` и добавлять его в каждую запись через `logging.Filter`. Для JSON используйте `python-json-logger` — он уже в зависимостях. Access-лог uvicorn отключите (`--no-access-log`), чтобы на запрос была одна строка.
...
3. **`GET /metrics`** в формате Prometheus (библиотека `prometheus-client`, уже в зависимостях):
   ...
   | `suggest_empty_total` | counter | — ; растёт на каждый ответ `/suggest` с пустой выдачей |
...
4. **Prometheus и Grafana в compose.** Добавьте сервисы и конфиги в `observability/`:
   ...
   - `observability/grafana/dashboards/app.json`: дашборд с `"uid": "app-overview"` и минимум четырьмя панелями — RPS по маршрутам, p95 времени ответа, доля ответов 5xx, пустые выдачи `suggest_empty_total` в секунду. Пятая панель с логами из Loki приветствуется. Дашборд удобно собрать в интерфейсе Grafana, а затем сохранить его JSON через экспорт дашборда и положить в этот файл.
   ...
   sum(rate(suggest_empty_total{job="app"}[1m]))
   ```

5. **Loki и Alloy.** Grafana Alloy находит контейнеры через Docker, читает их stdout и отправляет в Loki. Promtail для этого больше не используют: он устарел и с марта 2026 не поддерживается.
   ...
   Сервису `alloy` нужен сокет Docker, смонтированный только на чтение: `/var/run/docker.sock:/var/run/docker.sock:ro`. Имейте в виду: доступ к сокету — это полный доступ к Docker хоста. Локально это нормально, в проде так делают осторожно и осознанно.
...
- `/metrics` отдаётся; после 5 запросов `GET /suggest` счётчик с `path="/suggest"` вырос на 5 и больше; запрос на несуществующий путь не создаёт метку с сырым путём; есть `http_request_duration_seconds_bucket`; `suggest_empty_total` растёт после запроса с пустой выдачей;
...
PR `develop → main`, reviewer `vladefr97`, сообщение в чат: «project-autocomplete, ник, этап 4, PR готов». Подробно — в `Договоренности.md` курса.

## Типичные ошибки

- **Сырой путь в метке `path`** (`request.url.path`) — CI запросит несуществующий адрес, найдёт его в метках и упадёт.

...
Сервис по введённому префиксу возвращает подсказки для строки поиска: до K фраз из словаря, самые популярные сверху. Оператор загружает словарь без перезапуска, а выбор подсказки пользователем повышает её популярность.
...
- Работайте в ветке `develop`. В `main` напрямую ничего не пушьте.
- Каждый этап — отдельный pull request `develop → main`, в Reviewers — `vladefr97`. Когда CI зелёный, напишите в чат курса: «project-autocomplete, ник, этап N, PR готов».
- Этапы сдаются строго по порядку: следующий начинается после того, как предыдущий смержен. Этап 1 принимается до того, как вы пишете код сервиса.
- В начале каждого этапа со второго поднимите `stage` в `pyproject.toml` — это первый шаг каждого `TASK-N.md`. Если `stage` ниже сдаваемого этапа, CI не проверил нужные тесты: такой PR возвращается.
- PR с красным CI преподаватель не смотрит. Сначала зелёные проверки, потом ревью.
- `.github/` и `contract_tests/` не редактируйте. Изменения в них — повод вернуть PR без ревью.
- Перед пушем проверяйте локально: `make check`, а с этапа 2 ещё `make up` и `make test-contract`. Минуты GitHub Actions у организации общие на всех, а прогон с Docker идёт долго.
- Approve и merge делает преподаватель. Если нужны доработки, он напишет комментарии в PR: вы правите в `develop` и пушите, PR обновится сам.
- Когда CI зелёный, преподаватель смотрит глазами. На этапе 1 — архитектуру: оценка нагрузки посчитана, а не переписана из требований; в модели данных есть индексы; схема показывает потоки данных; выбор хранилища объяснён. Дальше — структуру кода, ваши тесты, нагрузочный отчёт, дашборд. Точный список — в «Критериях приёмки» каждого задания.

Сроки, оценка и правило про ИИ — в закреплённом сообщении чата курса.
